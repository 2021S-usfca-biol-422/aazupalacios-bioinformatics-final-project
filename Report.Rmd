...
In December of 2019, the first few cases of the novel SARS-CoV-2 virus began circulating around Wuhan, China. In the following months, the virus made its way across country boundaries and was eventually declared a worldwide pandemic by the World Health Organization on March 11th, 2020. The very first SARS-CoV-2 case in Spain was reported  on January 31st, 2020 in the Canary Islands, which are located off the coast of West Africa [@ferrer2020covid]. This first case from a German citizen was linked to a cluster of cases from Bavaria [@spiteri2020first]. Nine days later on February 9th, 2020 the first few cases were reported in the city of Palma on the island of Mallorca, in the Balearic Islands [@ferrer2020covid]. Then, on February 24th, 2020 the first cases of the virus were reported in the Spanish mainland [@ferrer2020covid]. Several weeks later, Spain, along with a couple other countries in western Europe, became the epicenter of the pandemic [henriquez2020first]. During this time, strict travel bans were set in place for international and national travel within Spain. 
...
```
**Figure 3**: Counts of samples from each of the geographic regions represented in the sample population. All regions in green are localities within the island of Mallorca, regions in blue come from the island of Ibiza, and regions in grey come from the Spanish capital, Madrid.
...
vcf_with_metadata %>%
  ...
  labs(title = "Count of Distinct SNPs in Known SARS-CoV-2 Genes from Madrid",
       ...
       y = "Count") +
  labs(fill = "Gene") +
  scale_fill_manual(values = c("black", "dodgerblue3", "grey50", "palegreen4", "cadetblue3", "darkseagreen", "thistle", "grey70",  "slateblue")) +
  theme(panel.grid.major = element_line(),
        panel.border = element_rect(fill = NA))
```
**Figure 4**: Frequencies of distinct SNPs of SARS-CoV-2 genes for samples collected from Madrid, Spain. 
...
vcf_with_metadata %>%
  ...
  mutate(`geographic_location_(region_and_locality)` = gsub(
    "Inca|Manacor|Palma|Santa Ponca", "Mallorca", `geographic_location_(region_and_locality)`)) %>% # change names of all
  ...
  theme(panel.grid.major = element_line(),
        panel.border = element_rect(fill = NA))
```
**Figure 5**: Frequencies of distinct SNPs of SARS-CoV-2 genes for samples collected from all localities within the island of Mallorca.
...
vcf_with_metadata %>%
  ...
  theme(panel.grid.major = element_line(),
        panel.border = element_rect(fill = NA))
```
**Figure 6**: Frequencies of distinct SNPs of SARS-CoV-2 genes for samples collected from all localities within the island of Ibiza.
...
```
**Figure 7**: Frequencies of the most common SNPs within the spike protein gene for each region. All SNPs represented here have frequencies of greater than 10 occurrences.
...
```
**Figure 8**: Frequencies of the most common SNPs within the nucleocapsid gene for each region. All SNPs represented here have frequencies of greater than 10 occurrences.
...
```
**Figure 9**: Frequencies of the most common SNPs within the ORF3a gene for each region. All SNPs represented here have frequencies of greater than 10 occurrences.
...
**Table 10**: Gene names, locations, and lengths in the SARS-CoV-2 genome. Higher SNP counts in the S and N genes may be related to the larger size of these genes.
```{r gene-table}
# create a table showing the length of each gene from start to end
gene_table %>%
