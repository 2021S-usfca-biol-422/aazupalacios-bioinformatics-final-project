...
vcf_with_metadata <- vcf_with_metadata[!vcf_with_metadata$sample == "ERR5704487", ]
```
...
``` {r spike-variant-Mainland-figure}
vcf_with_metadata %>%
  filter(!is.na(gene)) %>%
...
vcf_with_metadata %>%
  ...
  group_by(pos, gene, `geographic_location_(region_and_locality)`) %>%
  mutate(`geographic_location_(region_and_locality)` = gsub("_", " ", `geographic_location_(region_and_locality)`)) %>%
  mutate(`geographic_location_(region_and_locality)` = gsub("Inca|Manacor|Palma|Santa Ponca", "Mallorca", `geographic_location_(region_and_locality)`)) %>%
