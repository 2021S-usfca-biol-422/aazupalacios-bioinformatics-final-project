---
title: "Comparing SARS-CoV-2 variants between the Spanish capital and the Balearic Islands"
...
params:
  ...
  sra_runtable_path: "data/00_sra_runtable/SraRunTable_Balearic_Islands_Mainland.txt"
---
...
vcf_with_metadata %>%
  ...
  labs(title = "Count of samples by geographic region",
       ...
       y = "Count") +
  labs(fill = "Region") +
  scale_fill_manual(values = c("dodgerblue3",  "palegreen4", "grey50", "palegreen4",  "palegreen4", "palegreen4")) +
...
vcf_with_metadata %>%
  ...
  group_by(pos, gene, `geographic_location_(region_and_locality)`) %>%
  mutate(`geographic_location_(region_and_locality)` = gsub("_", " ", `geographic_location_(region_and_locality)`)) %>%
  mutate(`geographic_location_(region_and_locality)` = gsub("Inca", "Mallorca", `geographic_location_(region_and_locality)`)) %>%
    mutate(`geographic_location_(region_and_locality)` = gsub("Manacor", "Mallorca", `geographic_location_(region_and_locality)`)) %>%
    mutate(`geographic_location_(region_and_locality)` = gsub("Palma", "Mallorca", `geographic_location_(region_and_locality)`)) %>%
    mutate(`geographic_location_(region_and_locality)` = gsub("Santa Ponca", "Mallorca", `geographic_location_(region_and_locality)`)) %>%
...
vcf_with_metadata %>%
  ...
  mutate(`geographic_location_(region_and_locality)` = gsub(
    "Inca", "Mallorca", `geographic_location_(region_and_locality)`)) %>%
  mutate(`geographic_location_(region_and_locality)` = gsub(
    "Manacor", "Mallorca", `geographic_location_(region_and_locality)`)) %>%
  mutate(`geographic_location_(region_and_locality)` = gsub(
    "Palma", "Mallorca", `geographic_location_(region_and_locality)`)) %>%
  mutate(`geographic_location_(region_and_locality)` = gsub(
    "Santa_Ponca", "Mallorca", `geographic_location_(region_and_locality)`)) %>%
  group_by(pos, gene, `geographic_location_(region_and_locality)`) %>%
  filter(gene == "N") %>%
  tally() %>%
  filter(n >= 2) %>%
  ggplot(aes(x = pos, y = n, fill = `geographic_location_(region_and_locality)`)) +
  geom_col() +
  labs(title = "Count of SNPs per position in the S gene for each region",
       x = "Position",
       y = "Count") +
  labs(fill = "Region") +
  scale_fill_manual(values = c("dodgerblue3", "grey50", "palegreen4")) +
  theme(axis.text.x = element_text(angle = 90),
        panel.border = element_rect(fill = NA))
...
vcf_with_metadata %>%
  ...
  filter(`geographic_location_(region_and_locality)` != "Madrid") %>%
  filter(`geographic_location_(region_and_locality)` != "Ibiza") %>%
  mutate(Region = rep("Mallorca")) %>%
  group_by(ref, alt, pos, gene, Region) %>%
  filter(gene == "S") %>%
  tally() %>%
  knitr::kable(col.names = c("Reference",
                             "Alternate",
